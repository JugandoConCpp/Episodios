...
int Run_Test(char const* filename)
{
     ...
     std::println("Unit test has {} test groups.", count);
     ...
     for (auto const* test_group: test_groups)
     {
          for (auto const& test_function: test_group->Tests())
          {
               ...
               std::println("\x1b[0m {} {}", test_group->Name(), test_function.Name);
          }
     }

     std::println("{} tests run, {} passed, {} failed", run_count, pass_count, fail_count);
