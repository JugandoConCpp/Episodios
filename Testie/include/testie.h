...
namespace Testie
{
     ...
     inline bool Verify(
               ...
               bool expected_equal)
     {
          if (expected_equal && (actual != expected))
          {
               std::println("[{}:{}] Failed verification.", file, line);
               std::println(R"(     Expected: "{}")", expected);
               std::println(R"(     Actual: "{}")", actual);
               return false;
          }
          ...
          {
               std::println("[{}:{}] Failed verification.", file, line);
               std::println(R"(     Expected: NOT "{}")", expected);
               std::println(R"(     Actual: "{}")", actual);
               return false;
          }
...
#define VERIFY_ARE_EQUAL(expected, actual)                                                          \
     if (!Testie::Verify(__FILE__, __LINE__, expected, actual, true)) return false;                 \

#define VERIFY_ARE_NOT_EQUAL(expected, actual)                                                      \
     if (!Testie::Verify(__FILE__, __LINE__, expected, actual, false)) return false;                \
